/*
 * resonance-bpe.c — Field Resonance Architecture on BPE
 *
 * θ = ε + γ + αδ
 *   ε = trained weights
 ...
 *
 * BPE tokenizer with configurable vocab (default 2048).
 * Same architecture as resonance.c but with BPE in/out.
 *
 *   cc resonance-bpe.c -O2 -lm -o resonance-bpe
 *   ./resonance-bpe --train data.txt --depth 12 --bpe-vocab 2048
 *   ./resonance-bpe --generate --load resonance-bpe.bin --seed "The "
 *
...
static int g_vocab = 2048;  /* configurable, default BPE 2048 */
...
typedef struct {
    ...
    float embeds[2048][DF_DIM];  /* max BPE vocab */
    int   embed_init[2048];
...
static float *df_embed(int id) {
    if (id < 0 || id >= g_vocab) return NULL;
    if (!DF.embed_init[id]) {
...
static void df_overlay(float *logits, int V) {
    ...
    float h_max = 0;
    float *H_sig_buf = calloc(V, sizeof(float));
    ...
    float f_max = 0;
    float *F_sig_buf = calloc(V, sizeof(float));
    ...
    float *A_sig_buf = calloc(V, sizeof(float));
    ...
    free(H_sig_buf); free(F_sig_buf); free(A_sig_buf);
}
...
static int model_size(Cfg *c) {
    int s = c->V * c->E + c->T * c->E;  /* embeddings */
    int hw = hw_size(c);
    for (int b = 0; b < c->B; b++) {
        s += c->E;                  /* rms1 */
        s += c->H * hw;            /* attention */
...
static void model_init(Model *m, int depth) {
    ...
    m->cfg = cfg_from_depth(depth, g_vocab);
    Cfg *c = &m->cfg;
    ...
    float scale = sqrtf(2.0f / g_vocab);
    for (int i = 0; i < c->V * c->E; i++) m->w.tok_emb[i] = randn() * scale;
    ...
    scale = sqrtf(2.0f / c->E);
    for (int i = 0; i < c->E * c->V; i++) m->w.out_w[i] = randn() * scale;
...
static void acts_alloc(Acts *a, Cfg *c) {
    ...
    a->logits   = calloc(c->T * g_vocab, sizeof(float));
    a->probs    = calloc(c->T * g_vocab, sizeof(float));
}
...
static float forward(Model *m, Acts *a, const int *tokens, const int *targets) {
    ...
    matmul(a->logits, a->fln, w->out_w, T, E, g_vocab);

    /* Dario field overlay on logits — THE EQUATION */
    for (int t = 0; t < T; t++)
        df_overlay(a->logits + t * g_vocab, g_vocab);

    /* softmax + loss */
    float loss = 0;
    for (int t = 0; t < T; t++) {
        memcpy(a->probs + t*g_vocab, a->logits + t*g_vocab, g_vocab * sizeof(float));
        row_softmax(a->probs + t*g_vocab, g_vocab);
        if (targets) {
            float p = a->probs[t*g_vocab + targets[t]];
...
static void backward(Model *m, Acts *a, const int *tokens, const int *targets) {
    ...
    /* d_logits */
    float *dl = calloc(T*g_vocab, sizeof(float));
    for (int t = 0; t < T; t++)
        for (int v = 0; v < g_vocab; v++)
            dl[t*g_vocab+v] = (a->probs[t*g_vocab+v] - (v == targets[t] ? 1.0f : 0.0f)) / T;

    /* output projection */
    float *d_fln = calloc(T*E, sizeof(float));
    matmul_abt(d_fln, dl, w->out_w, T, g_vocab, E);
    matmul_atb(g->out_w, a->fln, dl, E, T, g_vocab);
    ...
    for (int b = B-1; b >= 0; b--) {
        ...
        for (int h = 0; h < H; h++) {
            ...
            *h_gate(g,c,b,h) += dg;
            ...
            for (int i = 0; i < T*T; i++) d_raw[i] *= sc;
...
static void train(Model *m, int *bpe_data, int bpe_len, int steps, float lr) {
    Acts a; acts_alloc(&a, &m->cfg);
    int T = m->cfg.T, V = m->cfg.V;
    int *tok = malloc(T * sizeof(int));
    int *tgt = malloc(T * sizeof(int));

    if (bpe_len <= T + 1) {
        fprintf(stderr, "[resonance-bpe] error: data too short (%d tokens, need > %d)\n",
                bpe_len, T + 1);
        exit(EXIT_FAILURE);
    }

    printf("[resonance-bpe] training: %d steps, lr=%.1e, %d tokens\n", steps, lr, bpe_len);
    clock_t t0 = clock();

    for (int s = 1; s <= steps; s++) {
        int off = rand() % (bpe_len - T - 1);
        for (int t = 0; t < T; t++) {
            tok[t] = bpe_data[off + t] % V;
            tgt[t] = bpe_data[off + t + 1] % V;
        }
...
static void generate(Model *m, const char *seed, int n, float temp) {
    ...
    /* BPE encode seed */
    int seed_tok[1024];
    int slen = bpe_encode((const unsigned char*)seed, strlen(seed), seed_tok, 1024);
    if (slen > T) slen = T;
    for (int i = 0; i < slen; i++)
        ctx[T - slen + i] = seed_tok[i];

    printf("%s", seed);
    for (int i = 0; i < n; i++) {
        forward(m, &a, ctx, NULL);
        float *logits = a.logits + (T-1) * V;
        int next = 0;
        if (temp <= 0.0f) {
            float best = logits[0];
            for (int v = 1; v < V; v++)
            ...
            if (temp != 1.0f)
                for (int v = 0; v < V; v++) logits[v] /= temp;
            row_softmax(logits, V);
        ...
        /* BPE decode and print */
        char decoded[32];
        int dlen = bpe_decode_token(next, decoded, 31);
        decoded[dlen] = 0;
        printf("%s", decoded);
        df_ingest(next);
...
static void model_save(Model *m, const char *path) {
    ...
    fclose(f);
    printf("[resonance-bpe] saved %s (%d params, vocab=%d)\n",
           path, m->n_params, m->cfg.V);
}
...
static void model_load(Model *m, const char *path) {
    ...
    if (!f) { fprintf(stderr, "cannot open %s\n", path); exit(1); }
    int magic;
    if (fread(&magic, 4, 1, f) < 1 || magic != 0x52425045) {
        fprintf(stderr, "[resonance-bpe] bad magic in %s\n", path); fclose(f); exit(1);
    }
    Cfg loaded;
    if (fread(&loaded, sizeof(Cfg), 1, f) != 1) {
        fprintf(stderr, "[resonance-bpe] failed to read config\n"); fclose(f); exit(1);
    }
    int nm;
    if (fread(&nm, 4, 1, f) < 1) { fclose(f); exit(1); }
    BPE.n_merges = nm; BPE.vocab_size = 256 + nm;
    if (fread(BPE.merges, sizeof(MergeRule), nm, f) < (size_t)nm) { fclose(f); exit(1); }
    g_vocab = loaded.V;
    ...
    if (fread(m->data, sizeof(float), m->n_params, f) != (size_t)m->n_params) {
        fprintf(stderr, "[resonance-bpe] truncated checkpoint\n"); fclose(f); exit(1);
    }
    if (fread(&DF, sizeof(DarioField), 1, f) != 1)
        printf("[resonance-bpe] warning: no field state\n");
    fclose(f);
    printf("[resonance-bpe] loaded %s (vocab=%d, field: %d cooc)\n", path, loaded.V, DF.cooc_n);
}
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (!strcmp(argv[i],"--bpe-vocab") && i+1<argc) g_vocab = atoi(argv[++i]);
    ...
    if (train_file) {
        ...
        /* BPE tokenize */
        int nm = g_vocab - 256;
        if (nm > MAX_MERGES) nm = MAX_MERGES;
        printf("[resonance-bpe] learning %d BPE merges...\n", nm);
        bpe_learn_merges(d.bytes, d.len, nm);
        printf("[resonance-bpe] BPE vocab: %d\n", BPE.vocab_size);
        int *bpe_data = malloc(d.len * sizeof(int));
        int bpe_len = bpe_encode(d.bytes, d.len, bpe_data, d.len);
        printf("[resonance-bpe] %d bytes -> %d tokens (%.2fx)\n", d.len, bpe_len, (float)d.len/bpe_len);
        free(d.bytes);
        train(&m, bpe_data, bpe_len, steps, lr);
